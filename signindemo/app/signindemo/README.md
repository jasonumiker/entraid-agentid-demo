...
On PowerShell, run `.\\.venv\\Scripts\\Activate.ps1` before developing.

On bash shells, run `source .venv/bin/activate` before developing.

To install just the runtime dependencies needed for local development, run `pip install -e .`.

To include the optional AWS OpenTelemetry bundle as well, run `pip install -e .[telemetry]`.
